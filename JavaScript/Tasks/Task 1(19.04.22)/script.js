...
const getSeason = (month = 'January') => {
  if (!monthList.includes(month)) {
    throw new Error('Nie ma takiego miesiaca'); // --> wyrzuca błąd jeśli nie ma takiego miesiąca w tablicy
  } else {
    switch (month) {
      case 'December':
      case 'January':
...
//? CODEWARS
//! Function reordering input numbers from the highest to the lowest value

//TODO: Create a function which checks if the number is negative integer (only takes non-negative integers)
//TODO: Function which takes an argument and returns it with the highest number posible in decreasing order
